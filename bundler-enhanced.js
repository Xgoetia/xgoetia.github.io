...
(function() {
    ...
    window.generateBundleScript = async function() {
        ...
        if (apps.length === 0) {
            alert(typeof t === 'function' ? t('noAppAlert') : 'Please select at least one app!');
            return;
        }
        ...
        if (format !== 'direct') {
            ...
            if (script) {
                ...
                alert(typeof t === 'function' ? t('downloadAlert') : 'Script downloaded!');
                return;
            }
        }
        
        // Direct download - open URLs in new tabs
        if (confirm(typeof t === 'function' ? t('directConfirm').replace('{n}', apps.length) : ('This will download ' + apps.length + ' files separately. Continue?'))) {
            apps.forEach(app => {
                window.open(app.url, '_blank');
            });
            alert(typeof t === 'function' ? t('directAlert') : 'Files are being downloaded.');
        }
    };
